...
export async function POST(req: Request) {
  try {
    ...
    if (shouldRefresh) {
      log.push(`Push: auto-refresh ETL for '${sourceDbName}'...`);
    ...
    log.push(`${logPrefix}: loading version + facility.mflcode`);

    // Resolve endpoint + auth (endpoint might be stored in global_property)
    const cbsCfg = loadCbsConfig();
    // Prefer the endpoint stored in the uploaded OpenMRS DB's global_property.
    // The env/default endpoint can be stale and cause HTTP 404 (no update / stale dashboard).
    ...
    try {
      ...
      // Phase 4 workload (IL-like): derive counts from OpenMRS Visits + Encounter types.
    ...
    // Values are placeholders for now; Phase 4 will populate real datasets.
    ...
    try {
      ...
      log.push(`Push: could not build visits dataset from ETL: ${String(err)}`);
    }
    ...
    try {
      ...
      log.push(`Push: could not build admissions dataset from ETL: ${String(err)}`);
    }
    ...
    const recordsToPush = {
      // Top-level datasets (count of array entries)
      ...
      // Placeholders that are currently empty (still included for audit)
    ...
    log.push(`CBS/Interop responded with HTTP ${lastStatus} (ok=${ok})`);

    return NextResponse.json(
      {
        ok,
        message: ok
          ? "Visualization payload pushed successfully"
          : "Visualization push failed",
        log,
        payload,
        pushSummary: {
          mfl_code: mflCodeFinal,
